...
if (!function_exists('safeSessionStart')) {
    ...
    function getRedirectUrl($role)
    {
        switch ($role) {
            ...
            case 'instructor':
                return '/webproject/instructor/index.php';
            case 'student':
                return '/webproject/student/index.php';
            default:
                return '/webproject/index.php';
        }
    }
    ...
    function getStudentByUserId(int $userId): array
    {
        $pdo = db();
        try {
            $st = $pdo->prepare("SELECT * FROM students WHERE user_id=? LIMIT 1");
            $st->execute([$userId]);
            $r = $st->fetch();
            return $r ?: [];
        } catch (Exception $e) {
